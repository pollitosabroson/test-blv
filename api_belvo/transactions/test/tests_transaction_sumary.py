...
from transactions.schemas import conf_schema_transaction_sumary
...
class TestSummaryUsers:
    ...
    url = lambda _, user_id: reverse(
        'transactions:summary_transaction',
        kwargs={
            'user_id': user_id
        }
    )
    ...
    def test_success(self, client):
        ...
        for fixtures in self.get_success_fixtures():
            ...
            for transaction in response_content:
                assert conf_schema_transaction_sumary.is_valid(transaction)
